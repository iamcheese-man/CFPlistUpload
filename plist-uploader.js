...
EXPIRY_TIME: 60 * 60 * 1000, // 1 hour in milliseconds
ALLOWED_EXTENSIONS: [’.plist’, ‘.mobileconfig’],
MAX_STORAGE: 1000, // Max number of files to store
};

// In-memory storage (resets on worker restart)
// In production, use KV or Durable Objects for persistence
const fileStorage = new Map();
// Map structure: { fileId: { content, filename, uploadTime, contentType } }
...
<head>
  ...
  <title>Plist File Uploader</title>
  <style>
...
<body>
  <div class="container">
    <h1>📄 Plist Uploader</h1>
    <p class="subtitle">Upload .plist or .mobileconfig files • Auto-delete after 1 hour</p>
...
<form id="uploadForm">
  <div class="upload-area" id="uploadArea">
    ...
    <input type="file" id="fileInput" accept=".plist,.mobileconfig" />
  </div>
...
<div class="stats" id="stats">
  ...
  <div class="stat-box">
    ...
    <div class="stat-label">Auto-Delete</div>
  </div>
</div>
...
<div class="info">
  <h3>ℹ️ Features</h3>
  <ul>
    <li>✅ View files in browser</li>
    <li>✅ Download anytime</li>
    <li>✅ Delete manually</li>
    <li>❌ No editing (read-only)</li>
...
<div class="api-section">
  <h3>🔌 API Usage</h3>
  <strong>Upload via cURL:</strong>
  <code>curl -X POST -F "file=@config.plist" ${self.location.origin}/upload</code>
  <strong style="display: block; margin-top: 15px;">Upload with JSON response:</strong>
  <code>curl -X POST -H "Accept: application/json" -F "file=@config.plist" ${self.location.origin}/upload</code>
</div>
```
  ...
  <script>
    ...
    fetch('/stats')
      .then(r => r.json())
      .then(data => {
        document.getElementById('statFiles').textContent = data.activeFiles;
        document.getElementById('statSize').textContent = data.storageUsed;
      });
    ...
    function handleFileSelect() {
      ...
      if (ext !== '.plist' && ext !== '.mobileconfig') {
        alert('Only .plist and .mobileconfig files are allowed');
        return;
      }
...
<div class="timer" id="timer">
  ⏰ Expires in: <strong>${minutes}m ${seconds}s</strong>
</div>
...
<div class="readonly-notice">
  ℹ️ <strong>Read-Only:</strong> This file cannot be edited. Download to modify locally.
</div>
```
  ...
  <script>
    ...
    function updateTimer() {
      if (timeLeft <= 0) {
        document.getElementById('timer').innerHTML = '⏰ <strong style="color: #d32f2f;">File expired</strong>';
        return;
      }
...
<body>
  <div class="box">
    <div style="font-size: 64px;">⏰</div>
    <h1>File Expired</h1>
    <p>This file has expired and been automatically deleted after 1 hour.</p>
    <a href="/">Upload New File</a>
  </div>
</body>
</html>`;
